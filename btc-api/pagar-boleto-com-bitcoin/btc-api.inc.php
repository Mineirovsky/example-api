...
class BitcoinAPI {
  ...
  *****************************************************************************/
  ...
  const CALLBK  = 'http://contascombitcoin.com.br/pagar-boleto-com-bitcoin/callback.php';
  ...
  public function registerBill($cod, $expire, $email) {
    /***************************************************************************
    Registra os dados da recarga para ser reconhecida posteriormente
    ***************************************************************************/
    $sqlite = new SQLite3('bills.sqlite');
    $sql = "INSERT INTO payments
                 VALUES ('$cod', '$expire', '$email', '$this->address', $this->xbt_value, $this->brl_value);";
    if(@$sqlite->query($sql)) {
      $sqlite->close();
      return true;
    } else {
...
$btc = new BitcoinAPI(100);
$btc->address = "dfg564fg6s5e4fsd54f";
$btc->registerBill('65410324365436352365032043684535000', '01/08/2016', 'amadobatista@cantor.com');
?>
